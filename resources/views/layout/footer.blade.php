<footer class="ftco-footer ftco-bg-dark">
        <div class="container" id="contact">
            <div class="row mt-5">
            <div class="col-md-3 col-sm-6 text-center pb-5">
                <img src="images/og.png" style="width:250px; margin-top:-50px;"></img>
                <div class="copyright text-center my-auto">
                    <span style="font-size:0.7em;" class="text-break">&copy;2022 OURGARAGE.ID</span>
                </div>
            </div>
            <div class="col-md col-6">
                <div class="ftco-footer-widget mb-4 ml-md-5">
                <h2 class="ftco-heading-2">Navigation</h2>
                <ul class="list-unstyled">
                    <li><a href="#" class="py-2 d-block">Home</a></li>
                    <li><a href="#" class="py-2 d-block">Our Items</a></li>
                    <li><a href="#" class="py-2 d-block">Services</a></li>
                    <li><a href="#" class="py-2 d-block">Term of Services</a></li>
                    <li><a href="#" class="py-2 d-block">Contact</a></li>
                </ul>
                </div>
            </div>
            <div class="col-md col-6">
                <div class="ftco-footer-widget mb-4 ml-md-5">
                <h2 class="ftco-heading-2">Kontak</h2>
                <ul class="list-unstyled">
                    <li><a href="#" class="py-2 d-block">Telepon</a></li>
                    <li class="kontak__">0895394620229</li>
                    <li><a href="#" class="py-2 d-block">Whatsapp</a></li>
                    <li class="kontak__">0895394620229</li>
                    <li><a href="#" class="py-2 d-block">Email</a></li>
                    <li class="kontak__">Ourgarage@gmail.com</li>
                </ul>
                <ul class="ftco-footer-social list-unstyled float-md-left float-lft">
                    <li class="ftco-animate"><a href="#"><span class="icon-twitter"></span></a></li>
                    <li class="ftco-animate"><a href="#"><span class="icon-facebook"></span></a></li>
                    <li class="ftco-animate"><a href="#"><span class="icon-instagram"></span></a></li>
                </ul>
                </div>
            </div>
            <div class="col-md col-12">
                <div class="ftco-footer-widget mb-4 align-item-center">
                    <h2 class="ftco-heading-2">Lokasi</h2>
                    <div class="block-23 mb-3">
                    <ul>
                        <li><span class="kontak__">OURGARAGE.ID</span></li>
                        <li><span class="icon icon-map-marker "></span><span class="kontak1__">Jl. Nangka Selatan No. 168 Denpasar - Bali</span></li>
                    </ul>
                    </div>
                </div>
            </div>

            </div>
        </div>
        </footer>

        <!-- SOSMED -->
